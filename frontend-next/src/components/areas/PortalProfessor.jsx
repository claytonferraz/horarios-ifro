...
export function PortalProfessor({
  ...
  const lastAutoSelectContext = React.useRef({ week: null, role: null });
   ...
   const filteredCourseClasses = React.useMemo(() => {
      ...
      return classes;
   }, [dynamicClassesList, scheduleMode, padraoFilterTeacher, showOnlyMyClasses, schedules, siape, dbClasses]);
    ...
    }, [selectedCourse, dbClasses, dbCourses, dynamicClassesList, showOnlyMyClasses, schedules, siape, mappedSchedules]);
   ...
   const estatisticasDocente = React.useMemo(() => {
                                      ...
                                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                         {aulasTurno.map((aula, idx) => {
                                           const isTarget = (() => {
                                             ...
                                             if (firstFutureFound && highlightedSubject && (String(aula.subject) === String(highlightedSubject))) return true;
                                           ...
                                           return (
                                              <div 
                                                key={aula.id || idx} 
                                                className={`group/item relative flex items-center justify-between p-5 rounded-[2rem] border transition-all duration-500 m-1 ${containerClasses} ${dashboardTab === 'vagas' ? 'cursor-pointer' : ''}`}
                                                onClick={() => {
                                                  if (dashboardTab === 'vagas' && isVaga) {
                    ...
                    <div className="flex flex-col gap-4">
                       <a href="https://suap.ifro.edu.br/edu/meus_diarios/" target="_blank" rel="noopener noreferrer" 
                          ...
                          <div className="text-left">
                            <h4 className={`text-sm font-black ${isDarkMode ? "text-white" : "text-slate-900"}`}>Meus Diários</h4>
                            <p className="text-[10px] font-bold text-slate-500 tracking-wider uppercase">Diários e Frequência</p>
                          </div>
                       </a>
                       ...
                       <a href="https://sei.ifro.edu.br/" target="_blank" rel="noopener noreferrer" 
                          ...
                          <div className="text-left">
                            <h4 className={`text-sm font-black ${isDarkMode ? "text-white" : "text-slate-900"}`}>SEI IFRO</h4>
                            <p className="text-[10px] font-bold text-slate-500 tracking-wider uppercase">Processos e Ofícios</p>
                          </div>
                       </a>
                       ...
                       <a href="https://suap.ifro.edu.br/admin/comum/sala/?agendavel__exact=1&all=&predio__uo=7&tab=tab_any_data" target="_blank" rel="noopener noreferrer" 
                          ...
                          <div className="text-left">
                            <h4 className={`text-sm font-black ${isDarkMode ? "text-white" : "text-slate-900"}`}>Reservar Salas</h4>
                            <p className="text-[10px] font-bold text-slate-500 tracking-wider uppercase">Agendamento SUAP</p>
                          </div>
                       </a>
