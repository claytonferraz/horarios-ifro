...
export const CourseGrid = React.memo(
  ({
    ...
    const [activeCourseTab, setActiveCourseTab] = useState(null);
    ...
    useEffect(() => {
      if (
        ...
        (!activeCourseTab || !availableCourses.includes(activeCourseTab))
      ) {
        setActiveCourseTab(availableCourses[0]);
      }
        ...
        <div className="flex flex-col lg:flex-row gap-4 animate-in zoom-in-95 duration-500">
          <div className="flex-1 space-y-6">
            {(() => {
              ...
              const course = activeCourseTab || availableCourses[0];
              const courseClassesGlobais = mappedSchedules
                .filter((r) => r.course === course)
                .map((r) => r.className);
              let courseClasses = [...new Set(courseClassesGlobais)].sort();
                    ...
                    <div className="flex flex-wrap gap-2 mb-2 no-print animate-in fade-in slide-in-from-top-2">
                      {availableCourses.map((c) => (
                        <button
                          ...
                          className={`px-4 py-2.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all shadow-sm ${course === c ? (scheduleMode === "padrao" ? "bg-blue-600 text-white ring-2 ring-blue-500/50 scale-[1.02] z-10" : scheduleMode === "previa" ? "bg-violet-600 text-white ring-2 ring-violet-500/50 scale-[1.02] z-10" : "bg-rose-600 text-white ring-2 ring-rose-500/50 scale-[1.02] z-10") : isDarkMode ? "bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700" : "bg-white text-slate-500 border border-slate-200 hover:text-slate-800"}`}
                        >
                          {c}
                        </button>
                      ))}
                  ...
                  {courseClasses.length === 0 ? (
                    ...
                    >
                      ...
                      <p
                        className={`text-sm font-medium mt-1 max-w-md mx-auto ${isDarkMode ? "text-slate-400" : "text-slate-500"}`}
                      >
                        Nenhuma turma atende aos critérios do filtro na matriz{" "}
                        <strong className="text-indigo-400">{course}</strong>.
                      </p>
                    </div>
                  ) : (
                    <div
                          ...
                          >
                            {safeDays.map((day, dayIndex) => {
                              ...
                              const displayShiftsDay = new Set(
                                activeTimes.map((i) => i.shift).filter(Boolean),
                              );
                              const activeIntervals = (intervals || []).filter(
                                (inv) => displayShiftsDay.has(inv.shift),
                              );
                              ...
                              return (
                                <React.Fragment key={`day-block-${day}`}>
                                  ...
                                  {dayIndex < safeDays.length - 1 && (
                                    ...
                                    >
                                      <td
                                        colSpan={activeCourseClasses.length + 2}
                                        className="py-1 shadow-inner"
                                      ></td>
                                    </tr>
                                  )}
                      ...
                      <div className="md:hidden no-print p-4 space-y-4">
                        {(() => {
                          const activeMobileCls =
                            mobileSelectedClasses[course] || courseClasses[0];
                            ...
                            <div className="animate-in fade-in zoom-in-95">
                              ...
                              {clsRecordsAll.length === 0 ? (
                                ...
                                <div className="space-y-4">
                                  {safeDays.map((day) => {
                                    const dayRecords = clsRecordsAll.filter(
                                      (r) => r.day === day,
                                    );
