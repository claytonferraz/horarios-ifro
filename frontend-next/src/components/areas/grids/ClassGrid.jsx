                                    ...
                                    {aulaNesteSlot ? (
                                      <div className="flex flex-col gap-1.5">
                                        {(() => {
                                          ...
                                          const hasPendingSwap = typeof checkPendingSwapRequest === 'function' && checkPendingSwapRequest(aulaNesteSlot);
                                          const isExtraPending = aulaNesteSlot.isPending === true && hasPendingSwap;
                                            ...
                                            <Draggable key={aulaNesteSlot.id || `dnd-${diaIndex}-${time}`} draggableId={String(aulaNesteSlot.id || `dnd-${diaIndex}-${time}`)} index={0} isDragDisabled={appMode === 'aluno'}>
                                              {(drgProvided, drgSnapshot) => (
                                                <div 
                                                  ...
                                                  className={`print-clean-card p-2 rounded-xl border shadow-sm flex flex-col justify-center min-h-[60px] transition-all relative ${drgSnapshot.isDragging ? 'z-50 scale-105 shadow-2xl rotate-2' : (isGridInert || isExtraPending ? 'cursor-default opacity-80' : 'hover:scale-[1.02] hover:shadow-md active:scale-95 cursor-pointer')} ${isExtraPending ? (isDarkMode ? 'bg-slate-800 border-dashed border-slate-500 text-slate-400' : 'bg-slate-100 border-dashed border-slate-400 text-slate-500') : hasPendingSwap ? (isDarkMode ? 'bg-amber-900/30 border-amber-800/50 hover:bg-amber-900/40 text-amber-200 shadow-[0_0_10px_rgba(251,191,36,0.2)]' : 'bg-amber-100 hover:bg-amber-200 border-amber-400 text-amber-900 shadow-[0_0_10px_rgba(251,191,36,0.2)]') : isPending ? (isDarkMode ? 'bg-red-900/30 border-red-800/50 text-red-300' : 'bg-red-50 border-red-300 text-red-800') : hasConflict ? (isDarkMode ? 'bg-rose-950/80 border-rose-500/80 text-rose-200 shadow-[0_0_10px_rgba(225,29,72,0.4)]' : 'bg-rose-100 border-rose-500 text-rose-900 shadow-[0_0_10px_rgba(225,29,72,0.3)]') : getColorHash(disciplineName, isDarkMode)}`}
                                                >
                                                  ...
                                                  {aulaNesteSlot.isPermuted && !isPending && (
                                                   ...
                                                   {aulaNesteSlot.classType && aulaNesteSlot.classType !== 'Regular' && !isPending && (
                                                      <div className="absolute bottom-0 right-0 z-10 pointer-events-none print:hidden">
                                                          <span className={`text-[6px] font-black uppercase tracking-wide text-white px-1.5 py-0.5 rounded-tl-[8px] ${isExtraPending ? 'bg-slate-500 border-slate-600' : 'bg-emerald-600 border-emerald-700'} border-l border-t block shadow-sm shadow-emerald-900/30`}>{isExtraPending ? 'ANÁLISE DAPE' : aulaNesteSlot.classType}</span>
                                                      </div>
                                                   )}
