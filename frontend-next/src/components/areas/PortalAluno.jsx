...
export function PortalAluno({
  ...
  const draggingRecord = null;
  const exchangeTarget = null;
  const exchangeAction = null;
  const pendingReverseTarget = null;
  ...
  const enrichScheduleItem = React.useCallback((s) => {
   ...
   const alunoSummary = React.useMemo(() => {
    ...
    const dashboardBaseWeekId = selectedWeek || (actualCurrentWeek ? String(actualCurrentWeek.id) : (academicWeeks.length > 0 ? String(academicWeeks[0].id) : null));
                    ...
                    <div className="flex flex-col gap-4">
                       <a href="https://suap.ifro.edu.br/" target="_blank" rel="noopener noreferrer" 
                          ...
                          <div className="text-left">
                            ...
                            <p className="text-[10px] font-bold text-slate-500 tracking-wider uppercase">Notas e Registros</p>
                          </div>
                       </a>
                       ...
                       <a href="https://wa.me/5569999047804" target="_blank" rel="noopener noreferrer" 
                          ...
                          <div className="text-left">
                            <h4 className={`text-sm font-black ${isDarkMode ? "text-white" : "text-slate-900"}`}>WhatsApp CAED</h4>
                            <p className="text-[10px] font-bold text-slate-500 tracking-wider uppercase">Suporte ao Aluno</p>
                          </div>
                       </a>
            ...
            <div className={`rounded-2xl shadow-sm border p-4 space-y-4 no-print print:hidden ${viewMode === 'dashboard' ? 'hidden' : ''} ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
              ...
              {viewMode !== 'curso' && viewMode !== 'sem_professor' && (
                <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 p-3 rounded-xl border no-print ${isDarkMode ? 'bg-slate-900/50 border-slate-800' : 'bg-slate-50 border-slate-100'}`}>
                  ...
                  {(viewMode === 'professor' || viewMode === 'outro_professor') && (
              ...
              {(['turma', 'hoje', 'historico'].includes(viewMode)) && selectedClass && (
